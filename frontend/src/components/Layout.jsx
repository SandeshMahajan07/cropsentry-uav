...
export default function Layout({ activeTab, setActiveTab, children, onRefreshData, onGoHome }) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F6F2] text-slate-800 flex flex-col md:flex-row">
      {/* Sidebar Desktop — Sticky & Constant */}
      <aside className="hidden md:flex flex-col w-72 bg-[#122418] text-white border-r border-[#1e3827] shadow-xl shrink-0 z-30 sticky top-0 h-screen overflow-y-auto">
        {/* Brand Header */}
        <div className="p-6 border-b border-forest-900/60 shrink-0">
        ...
        <div className="p-4 border-t border-forest-900/60 space-y-2.5 shrink-0">
          <button
            onClick={() => setIsWhatsAppModalOpen(true)}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-bold transition shadow-md border border-emerald-600 active:scale-[0.98]"
          >
          ...
          >
            ...
            <span>Simulate Drone Detection</span>
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Header */}
        <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsWhatsAppModalOpen(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-700 hover:bg-emerald-600 text-white text-xs font-bold transition shadow-sm"
